...
class UserFollowModel  extends  Model {
	...
	public function  unFollow($from_uid, $to_uid){
		$is_follow = $this->checkFollow($from_uid, $to_uid);
		if ($is_follow) {
			$this->where(array('from_uid'=>$from_uid, 'to_uid'=>$to_uid, 'status'=>1))->delete();
			return  true;
		} else{
			return  false;
		}
	...
	public  function   getFansNum($uid){
		$result = $this->where(array('to_uid'=>$uid, 'status'=>1))->count();
		return $result > 0 ? $result : 0;
	}
	public  function   getFollowNum($uid){
		$result = $this->where(array('from_uid'=>$uid, 'status'=>1))->count();
		return $result > 0 ? $result : 0;
	}
	...
	public  function   getUserFan($uid){
		$result['data'] = $this->where(array('to_uid'=>$uid, 'status'=>1))->order('create_time DESC')->selectPage(10);
		
		$result['data'] = $this->getRelUser($result['data'], 'fan');
		if ( !empty( $this->page ) ) {
	...
	public  function  getUserFollow($uid){
		$result['data'] = $this->where(array('from_uid'=>$uid, 'status'=>1))->order('create_time DESC')->selectPage(10);
		$result['data'] = $this->getRelUser($result['data'], 'follow');
		if ( !empty( $this->page ) ) {
			$result['page']  = $this->page;
		}
		return $result;
